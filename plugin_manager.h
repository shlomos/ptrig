...
struct plugin_manager *init_plugin_manager(const char *);
...
struct list_head *get_module_hooks(struct plugin_manager *, const char *);

/* Get all general hooks from loaded plugins */
struct list_head *get_general_hooks(struct plugin_manager *);

/* Load all plugins. Call after registering all modules */
int load_plugins(struct plugin_manager *);
